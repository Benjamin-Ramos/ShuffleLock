...
const Uploader = ({ setImages }) => {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: '50px', marginBottom: '20px', opacity: isDragging ? 1 : 0.4, color: isDragging ? '#facc15' : '#fff' }}>📦</div>
      <h3 style={{ color: '#fff', fontSize: '18px', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '10px' }}>{isDragging ? 'Suelta para Añadirla' : 'Suministra tus opciones'}</h3>
      <p style={{ color: '#666', fontSize: '12px', marginBottom: '30px', textTransform: 'uppercase' }}>O arrastra tus cartex</p>
      <input type="file" multiple accept="image/*" onChange={handleChange} id="file" style={{ display: 'none' }} />
      <label htmlFor="file" style={{ backgroundColor: '#facc15', color: '#000', padding: '14px 35px', fontWeight: '900', cursor: 'pointer', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px', boxShadow: '0 4px 0 #b48608', display: 'inline-block' }}>Seleccionar Imagenes</label>
      {fileCount > 0 && <div style={{ marginTop: '25px', color: '#4b69ff', fontSize: '11px', fontWeight: 'bold', textTransform: 'uppercase' }}>● {fileCount} Imágenes listas</div>}
    </div>
  );
};
